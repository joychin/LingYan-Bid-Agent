...
SUBAGENTS: list[dict] = [
    ...
    },
    {
        ...
        "system_prompt": (
            "你是响应文件正文编写子代理，只负责任务描述点名的目录节的正文。"
            "任务描述=首行节名清单（一节或多节，多节以顿号分隔）"
            ...
            "开局纪律：任务描述的「写作纪律」段已附写作方法论（section-writing.md 全文），"
            "**不要再 read_file 它**；若该段缺失（长描述的最简拼装路径），开工前"
            " read_file skills/tender-body/references/section-writing.md 一次读齐；"
            "若还需其它参考文件，同样在同一条消息里一次读齐"
            "（禁止逐个串行）；禁止调用"
            ...
            "来源文件/区间，够做规划，**不再调用 search_references**；清单缺失、指引标"
            "【缺】或块标已失效才自行检索）→ 列使用计划（记下每块用整块还是只用块内"
            "哪些行号区间）→ docx_section_create 建节 + docx_material_inject 保真贴底稿"
            "（块是拷贝授权范围非注入原子：本节只需要块内某些章节时传 lines 行号区间"
            "只注入相关部分，整块适用则不传；非 docx 素材才自行撰写）→ "
            "docx_section_read 拿段号 + docx_section_revise 改写适配本项目 → "
            "validate_body(section=<节路径>, block_ids=<使用计划的块，只注入了块内"
            "区间的写 blk_…:L起-L止>) 自查。\n"
            ...
            "素材块内的图片只有注入能带进正文（自写=图全丢）；注入粒度按本节需要"
            "挑选（整块或块内相关区间），同一内容区间不得进两节（其他节也在用同一"
            "块时尤其注意）；认为指引指派的素材块"
...
def _kb_summary_line() -> str:
    ...
    if mt_files:
        ...
        lines.append(
            f"写作素材库 {len(mt_files)} 份文件 {n_blocks} 个素材块（用户手工挑选的章节+备注，拷贝修订的首选底稿）"
        )
    return (
        "### 公司知识库与写作素材\n" + "\n".join(lines) + "\n"
        "两库内容都可改写进正文，裁决规则：同一主题素材库有块→用块做拷贝底稿"
        "（docx_material_inject 注入后修订，块是拷贝授权范围、可只注入本节需要的"
        "行号区间），知识库只补块没有的东西（证书原图、时效核对、未覆盖的事实），"
        "已注入块的节不要再把知识库同主题文本整段转述进正文；没有块的主题→"
        "search_company_assets 检索后按行号区间 read_file 精读，摘事实写成正文"
        "（数字/名称逐字保真，历史标书业绩描述须与合同核对，拟投入承诺不是现状"
        "事实）；需要整章拷贝修订、或想知道某类产品/服务该写哪些能力模块时用 "
        "search_references（查用户手工挑选的素材块及其备注；素材带历史项目语境，"
        "数字与承诺须按本次招标重新核对；拷贝素材后必须 check_name_residue 扫旧名残留）。"
        "同一内容区间不得进两节（节间查重兜底）。"
    )
...
def build_agent(profile: cfg.ModelProfile | None = None):
    ...
    agent = create_deep_agent(
        ...
        system_prompt=(
            ...
            "（只润模型自己生成的文字；素材底稿与拷贝的招标格式件保真不润，"
            "细则见 tender-body 语言纪律）。"
            ...
            "（资质/案例/业绩，检索核对用）；写作素材库=用户手工挑选的章节素材块"
            "（内容资产，拷贝修订的首选底稿——同主题优先于知识库转述；块是拷贝"
            "授权范围，注入粒度按节自选）；版式库=纯版式资产（原「模板库」），只决定之后新建节与"
