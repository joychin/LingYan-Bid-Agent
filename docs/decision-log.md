...
- L1131 起 *skill 沿革日期清洗批（2026-09-18，模型语境去变更史：三分判据 21 处+防回流测试）：
...
- **skill 沿革日期清洗批（2026-09-18，模型语境去变更史：21 处+防回流测试，纯文案）**：
  动因=用户审 tender-body/SKILL.md 圈出「派发分组自 2026-09-15 起由你自主规划——
  程序不再给出分好波的参考」问这类语句是否合适。裁定=不合适且是一类：「分好波的
  参考」是模型从未见过、现已被删的概念，被引入只为否定它——模型既读不出可用信息，
  还可能（a）去找一个不存在的计划（b）把内部变更讲给用户。与「内部编号退出写手
  语境」（2026-09-08 REQ 泄漏根治批）同一条道理：**版本沿革同样不在模型语境**。
  三分判据：**证据型**（日期标记一次实测后果，如「2026-09-08 实测：整本自写=0 图」）
  **留**——硬规则挂具体后果模型执行更好；**沿革型**（日期标记我们改过主意：「自 X
  起／此前／接线／拍板／X 批」）**删从句留规则**；**事实型**（世界里的时间点：证书
  有效期、递交截止示例）**必须留**。清洗面不止 skill 文件——凡进模型上下文的物件
  同罪，共 21 处：skill 17（SKILL.md 7／section-writing 4／guide-format 5／
  evidence-rules 1〔文件搬迁史整句删〕）+ 代码侧 4（写手子代理 system_prompt 的
  「2026-09-15 起一个任务可能带多个节」——每次派发都进上下文；docx_ops 三处
  @tool docstring 的「批二/批三/表格通道批」——工具描述每轮随 schema 下发，暴露
  频次比 SKILL 还高）。混合型两处只删日期留后果（罗列清单塌成五字节名／格式件
  空格漏填=空格子交付）。顺带修一处**无日期的同类沿革措辞歧义**：section-writing
  尾句「分组与排序不再是模型罗列上下文的职责」——「分组」现在是模型的核心职责
  （09-15 拆分归模型拍板），原句会被反着读，改「（共享上下文）无需模型罗列」。
  防回流测试两处：test_skills 扫全部 skill .md 的 20xx-xx-xx 白名单（5 处=证据型
  3+事实型 2，多出即红；白名单按行片段匹配，跨行括注要拆两条）；test_agent 子代理
  spec 循环加「system_prompt 不含 20\d\d-\d\d」。开发者侧注释/模块 docstring/
  测试 docstring 的日期**全部保留**——那是沿革的合法居所（decision-log 之外的
  存档处），与「给人读的 AGENTS.md vs 给模型的 skill」分工同理。缓存影响同上批：
  改 system prompt/docstring=前缀缓存一次性作废，集中单次部署+重启 sidecar
  （skills 镜像+_skill_cache+agent 重建）。明确不做：humanizer-zh 第三方件不动
  （[日期] 是占位模板、1989 是教学例句）；主 prompt 与 guard 返回语核查无日期
  零改动；不带日期的沿革措辞普查（此前/不再/旧版/已废/拍板/接线）命中全为当前
  语义误报（「用户拍板」=用户做决定、「旧版自动入恢复点栈」=文件的上一版、
  「不再调用工具的那一轮」=当轮描述），不清洗。
...
- **提问卡原位替换输入框（2026-09-09，用户拍板方向 A，文生图 mockup 定稿）**：
  ...
  **底部输入框槽位原位替换 InputComposer**（ChatView 条件渲染 `interrupt && !running`；
  ...
  - **宽面板覆盖聊天输入区修复——ap-slot 占位与浮层同宽（2026-09-23，自动化测试 N2）：**
    ...
    原样生效（wsWidth state 双向消费）。Review 补刀（同日）：占位同宽把覆盖式变真实让位后，
    wsMaxW 旧口径「视口-360」没扣侧栏（窄态 navMaxW 扣 264 的先例没跟上）——拖到上限聊天
    仅剩 ~96px；上限改「视口-264-360」、默认值同样过 clamp（60% 视口被夹入上限内，任何
    窗口默认态聊天 ≥360；视口 <1264 时 640 下限顶住、聊天收窄但完整可用）。明确不做：wide
    阴影收窄（box-shadow 不拦截指针，分层感保留）。验证=vitest 345 绿 + Playwright 复现
    翻转：修复前 elementFromPoint(发送钮中心) 命中 docx 预览层，修复后真实点击成功发出消息，
    实拍面板与聊天区并排、输入区完整。
