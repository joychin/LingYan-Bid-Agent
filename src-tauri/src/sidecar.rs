...
//!   3. 退出时杀整个进程树
//!   4. 提供 commands：get_sidecar_info / get_llm_settings / set_llm_settings / get_api_key_has_value
...
pub struct SidecarManager {
    ...
    pub settings: Mutex<LlmSettings>,
...
/// 读 settings.json；缺失/损坏返回 None（调用方回退到默认设置）。
pub fn read_settings_file() -> Option<(String, String)> {
    ...
    Some((base.trim().to_string(), model.trim().to_string()))
}

/// 写 settings.json（set_llm_settings 持久化，使 HTTP PUT 与 IPC 走同一真值）。
pub fn write_settings_file(base_url: &str, model: &str) -> Result<(), String> {
    let path = settings_file_path();
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| e.to_string())?;
    }
    let json = serde_json::json!({ "base_url": base_url, "model": model });
...
/// 故改用 CLI 子进程以满足 PRD 验收（service=tender-agent，account=llm-api-key）。
pub fn keychain_set(key: &str) -> Result<(), String> {
    let out = Command::new("security")
        .args(["add-generic-password", "-U", "-s", KEYRING_SERVICE, "-a", KEYRING_ACCOUNT, "-w", key])
        .output()
        .map_err(|e| e.to_string())?;
...
pub fn keychain_has_value() -> bool {
    keychain_get().map(|k| !k.is_empty()).unwrap_or(false)
}

fn resolve_api_key() -> Option<String> {
    // 优先钥匙串；开发期回退环境变量（sidecar/.env 由 tauri dev 的 shell 继承不到，故再兜底）
    if let Ok(k) = keychain_get() {
        if !k.is_empty() {
            return Some(k);
        }
    }
...
fn spawn_sidecar(api_key: Option<String>, base_url: &str, model: &str, nonce: &str) -> std::io::Result<(Child, u16, String)> {
...
pub fn run_supervisor(app: AppHandle, mgr: Arc<SidecarManager>) {
    thread::spawn(move || {
        ...
        'outer: loop {
            ...
            let (base_url, model) = read_settings_file().unwrap_or_else(|| {
                let s = mgr.settings.lock().unwrap();
                (s.base_url.clone(), s.model.clone())
            });

            let nonce = Uuid::new_v4().to_string();
            match spawn_sidecar(resolve_api_key(), &base_url, &model, &nonce) {
                Ok((child, port, token)) => {
                    ...
                    *mgr.state.lock().unwrap() = SidecarState::Starting;
                    ...
                    if wait_healthy(port, &nonce) {
                        ...
                        *mgr.state.lock().unwrap() = SidecarState::Running;
                        ...
                        log::info!("sidecar running on 127.0.0.1:{port} (base={base_url} model={model})");
                    } else {
                        if mgr.stopping.load(Ordering::SeqCst) {
                            kill_and_reap(&mgr);
                            break 'outer;
                        ...
                        *mgr.state.lock().unwrap() = SidecarState::Failed;
                        ...
                        if failed >= MAX_RESTARTS {
                            ...
                            // 等待手动触发（set_llm_settings 会置 restart_requested）
